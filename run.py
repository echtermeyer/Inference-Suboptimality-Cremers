...
parser.add_argument(
    "--ais-schedule", type=str, default="linear", help="schedule for AIS"
)
...
parser.add_argument(
    "--encoder-more", action="store_true", help="train the encoder more (5 vs 1)"
)
parser.add_argument(
    "--early-stopping", "-es", action="store_true", help="apply early stopping"
)
parser.add_argument(
    "--epochs",
    "-e",
...
parser.add_argument(
    "--lr-schedule", "-lrs", action="store_true", help="apply learning rate schedule"
)
...
parser.add_argument(
    "--batch-size", "-bs", type=int, default=100, help="batch size (default: 100)"
)
parser.add_argument(
    "--save-name",
    "-sn",
...
parser.add_argument(
    "--has-flow", "-hf", action="store_true", help="use flow for training and eval"
)
...
parser.add_argument(
    "--warmup", "-w", action="store_true", help="apply warmup during training"
)
    ...
    for epoch in tqdm(range(1, epochs + 1)):
        ...
        if epoch % display_epoch == 0:
            ...
            for _, (batch, _) in enumerate(train_loader):
                ...
                train_logpx.append(logpx.item())
                train_logpz.append(logpz.item())
                train_logqz.append(logqz.item())
            ...
            for _, (batch, _) in enumerate(test_loader):
                ...
                test_logpz.append(logpz.item())
                test_logqz.append(logqz.item())
            print(
...
def test_iwae(model, loader, k=5000, f="model.pth", print_res=True):
...
def run():
    ...
    if args.train:
        save_path = "checkpoints/%s/%s/%s%s/" % (
            ...
            (
                ...
                else "flow" if args.has_flow else "ffg"
            ),
        )
    ...
    if args.visdom:
        ...
        win_samples = vis.images(
            images, 10, 2, opts={"caption": "original images"}, win=None
        )
        ...
        win_samples = vis.images(
            images, 10, 2, opts={"caption": "reconstruction"}, win=None
        )
    ...
    if args.eval_ais:
        model.load_state_dict(torch.load(args.eval_path)["state_dict"])
        schedule_fn = (
            linear_schedule if args.ais_schedule == "linear" else sigmoidial_schedule
        )
